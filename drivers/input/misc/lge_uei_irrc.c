...
static int uei_irrc_probe(struct platform_device *pdev)
{
	...
	if(lge_get_board_revno() == HW_REV_C ) {
		rc = gpio_request(GPIO_IRDA_PORT_TX, "RC_IRDA_TX_EN");
		if(rc)
				pr_err("%s: This port is busy (%d)\n", __func__, GPIO_IRDA_PORT_TX);
		else {
			gpio_tlmm_config(GPIO_CFG(GPIO_IRDA_PORT_TX, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
			gpio_set_value(GPIO_IRDA_PORT_TX,0);
		}
	}
#endif
        return 0;
