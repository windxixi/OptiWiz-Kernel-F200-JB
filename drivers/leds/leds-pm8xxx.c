...
enum modem_led_status_type{
INDICATOR_LED_BLINKING=0,
INDICATOR_LED_TIMER_CLR
};
...
static void led_lc_indicator_set(struct pm8xxx_led_data *led, enum kernel_led_status_type value)
{
	...
	case LED_SET_BLINKING:
		led_status_indicator = INDICATOR_LED_BLINKING;
		break;
	case LED_CLEAR_CP_TIMER:
		led_status_indicator = INDICATOR_LED_TIMER_CLR; 
		break;		
	}	
	...
	enum led_brightness value)
{
	...
	if(led->id == PM8XXX_ID_LED_0){
		...
		if(led_operation_mode == START_MODE)
		{
			...
			if(led->cdev.brightness == LED_SET_BLINKING)
			{
		...
		else if(led_operation_mode == WITH_NOTI_MODE)
		{
			...
			if( !(led->cdev.brightness == LED_SET_BLINKING) )
			{
				led_lc_indicator_set(led,LED_CLEAR_CP_TIMER);
