...
int8_t ce1702_set_wb_setting(struct msm_sensor_ctrl_t *s_ctrl, uint8_t wb)
{
	...
	switch (wb) {
		...
		case CAMERA_WB_FLUORESCENT:
			wb_mode = 0x01;
			data[0] = 0x10;
#if defined(CONFIG_MACH_APQ8064_GKATT) ||defined(CONFIG_MACH_APQ8064_GKOPENHK)||defined(CONFIG_MACH_APQ8064_GVDCM)
			data[1] = 0x05;
			LDBGI("%s: setting CAMERA_WB_FLUORESCENT for E980 & L-04E value = %d\n", __func__, data[1]);
#else
			data[1] = 0x03;
			LDBGI("%s: setting CAMERA_WB_FLUORESCENT for F240 value = %d\n", __func__, data[1]);
#endif			
			break;
