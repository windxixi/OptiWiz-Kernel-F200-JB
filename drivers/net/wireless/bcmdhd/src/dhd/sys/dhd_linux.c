	...
	if (ifidx == ALL_INTERFACES) {
		...
		for (i = 0; i < DHD_MAX_IFS; i++) {
			if (dhd->iflist[i]) {
				...
				/* LGE_CHANGE_S moon-wifi@lge.com by kwisuk.kwon 120111 [start] : prevent a kernel panic */
				#ifdef LGE_BCM433X_PATCH
					if( !net ) continue;
				#endif //LGE_BCM433X_PATCH
				/* LGE_CHANGE_E moon-wifi@lge.com by kwisuk.kwon 120111 [end] */				
				if (state == ON)
					netif_stop_queue(net);
				else
