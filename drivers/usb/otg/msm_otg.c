...
#ifdef CONFIG_LGE_PM
#if defined(CONFIG_USB_G_LGE_ANDROID) && !defined(CONFIG_MACH_APQ8064_GKATT) && !defined(CONFIG_MACH_APQ8064_GK_KR) && !defined(CONFIG_MACH_APQ8064_GKOPENHK)
...
#if !(defined(CONFIG_MACH_APQ8064_GKATT) || defined(CONFIG_MACH_APQ8064_GKOPENHK))  // leechangjun 20121211 for GKATT HW REV_D add 
        if((boot_mode != LGE_BOOT_MODE_FACTORY2) && (lge_get_board_revno()==HW_REV_C || lge_get_board_revno() >= HW_REV_1_0))
			//if(!slimport_is_connected())
				usb_id_sel_enable(0);
#else
