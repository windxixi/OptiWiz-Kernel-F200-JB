...
void lm3533_late_resume(struct early_suspend * h)
{
	...
	for(i=0;i<10;i++)
	{
		if(	msm_fb_bl_workqueue_exected)
		{
	...
	pr_info("%s[Start] backlight_status: %d requested_in_early_suspend_lcd_level: %d\n", __func__, backlight_status,requested_in_early_suspend_lcd_level);
